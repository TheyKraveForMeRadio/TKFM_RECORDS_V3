...
exports.handler = async (event) => {
  try {
    ...
    if (action === "create") {
      ...
      artistCredits[artistEmail] = (artistCredits[artistEmail] || 0) + 0; // starter credits
    ...
    if (action === "list") {
      const list = Object.entries(deliverables)
        .filter(([_, d]) => d.artistEmail === artistEmail)
        .map(([id, d]) => ({ id, ...d }));

...
// netlify/functions/owner-financials.js
var owner_financials_exports = {};
__export(owner_financials_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(owner_financials_exports);
...
async function handler(event) {
  const ownerKey = event.headers["x-owner-key"];
  if (ownerKey !== process.env.TKFM_OWNER_KEY) {
    return failure("Unauthorized", 401);
  }
  try {
    const { count: totalArtists } = await supabase.from("tkfm_artists").select("*", { count: "exact", head: true });
    const { count: activeSubs } = await supabase.from("tkfm_artists").select("*", { count: "exact", head: true }).eq("subscription_active", true);
    const { data: creditsUsed } = await supabase.from("tkfm_credit_ledger").select("amount").eq("action", "debit");
    const { data: creditsIssued } = await supabase.from("tkfm_credit_ledger").select("amount").eq("action", "credit");
    const totalUsed = creditsUsed?.reduce((sum, r) => sum + Number(r.amount), 0) || 0;
    const totalIssued = creditsIssued?.reduce((sum, r) => sum + Number(r.amount), 0) || 0;
    const { data: revenueData } = await supabase.from("royalty_events").select("revenue");
    const totalRevenue = revenueData?.reduce((sum, r) => sum + Number(r.revenue), 0) || 0;
    const { data: recentLedger } = await supabase.from("tkfm_credit_ledger").select("*").order("created_at", { ascending: false }).limit(20);
    return success({
      ...
      recent_financial_activity: recentLedger || []
    });
  } catch (err) {
...
//# sourceMappingURL=owner-financials.js.map

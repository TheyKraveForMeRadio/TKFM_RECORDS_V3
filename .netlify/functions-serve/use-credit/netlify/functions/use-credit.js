...
var use_credit_exports = {};
__export(use_credit_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(use_credit_exports);
...
async function handler(event) {
  ...
  try {
    ...
    const { data: artist, error } = await supabase.from("tkfm_artists").select("credits").eq("email", email).single();
    ...
    if (currentBalance < amount) {
      return failure("Insufficient credits");
    }
    const newBalance = currentBalance - amount;
    credits[credit_type] = newBalance;
    await supabase.from("tkfm_artists").update({ credits }).eq("email", email);
    await supabase.from("tkfm_credit_ledger").insert({
      email,
      action: "debit",
      credit_type,
      amount,
      balance_after: newBalance,
      metadata: { source: "use-credit" }
    });
    return success({
      credit_type,
      balance_after: newBalance
...
//# sourceMappingURL=use-credit.js.map
